...
export const getDistanceFromLatLon =
  (origin: Coordinates, destination: Coordinates): number => {
    const earthRadius = 6371; // In km
    const differenceLatitude =
      degreesToradius(destination.latitude - origin.latitude); // deg2rad below
    const differenceLongitude =
      degreesToradius(destination.longitude - origin.longitude);
    const a =
